...
2. **Execute the sprints in order** (00 → 08). Each sprint has *Entry Criteria* (verify before starting) and a *Definition of Done* (verify before closing). Do not mix sprint scopes.
...
3. **Every test is already specified** with an ID, "what it validates", entry criteria, and exit criteria. Implement exactly what is specified (you may add tests, never remove any) and name each test with its ID: `test('S01-UT-01: ...')`.
4. When a sprint is finished, write the report in `docs/reports/sprint-XX-report.md` with the evidence required by the DoD.
5. When in doubt between documents: `project-rules.md` > the sprint > the other documents.
...
| [`project-rules.md`](project-rules.md) | Gates G1–G6, dependency rule, BR-01..BR-10, Git workflow, prohibitions |
...
|---|---|---|
| [00 — Setup](sprints/sprint-00-setup.md) | Flutter project, theme/design system, fakes, CI, import check | S00: 4 UT · 2 GT · 1 IT · 1 E2E |
